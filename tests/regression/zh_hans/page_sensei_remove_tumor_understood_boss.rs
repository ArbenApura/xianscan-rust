...
fn test_regression_page_sensei_remove_tumor_understood_boss() {
    ...
    // 4. PANEL 1 TOP-MIDDLE SLANTED FREE TEXT: '老大！明白！'
    let ft3 = res.regions.iter().find(|r| r.text.contains("老大") || r.text.contains("明白"));
    assert!(ft3.is_some(), "Must detect top-middle slanted free text '老大！明白！'");
    let ft3 = ft3.unwrap();
    assert_eq!(ft3.kind, RegionKind::FreeText);
    assert_eq!(ft3.bubble_box, None, "Free text must not be associated with a bubble envelope");
    crate::assert_region_bounds!(ft3, RegionKind::FreeText, 319, 153, 66, 72, 20);
    crate::assert_region_angle!(ft3, -5.44, 2.0);
