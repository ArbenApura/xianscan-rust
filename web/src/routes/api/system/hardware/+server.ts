...
export const GET: RequestHandler = async () => {
	try {
		...
		if (pipeline.getHardware) {
			...
			return json({
				...hw,
				version: hw.version || '0.5.0-beta.6',
				app_version: hw.app_version || '0.5.0-beta.6+dev',
		...
		return json({
			...
			has_directml: false,
			has_coreml: false,
			version: health.version || '0.5.0-beta.6',
			app_version: health.app_version || '0.5.0-beta.6+dev',
