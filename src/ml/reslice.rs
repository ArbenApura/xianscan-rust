...
pub fn find_forbidden_text_zones(
    ...
    while y_top < total_h {
        ...
        if cur_tile_h >= 32 {
            let tile = canvas.crop_imm(0, y_top, canvas.width(), cur_tile_h);

            // 1. Comic text detector
            if let Some(ref mut det) = detector {
                if let Ok(res) = det.detect(&tile) {
                    for box_poly in res.boxes {
                        ...
                        let y_min = (by + y_top as i32 - safety_margin).max(0);
                        let y_max = (by + bh + y_top as i32 + safety_margin).min(total_h as i32);
                        raw_intervals.push((y_min, y_max));
                    }
                }
            }
...
pub fn find_optimal_cut_points(
    ...
    while current_y < total_h {
        ...
        // 3rd Pass: Fallback to ideal search range
...
pub fn smart_reslice_chapter(
    ...
    let cut_points = find_optimal_cut_points(&stitched, target_height, min_height, max_height, &forbidden_zones);
