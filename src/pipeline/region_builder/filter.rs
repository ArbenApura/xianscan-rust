...
pub fn should_reject_candidate_region(
    ...
    let avg_char_dim = if cluster_rect.h >= cluster_rect.w {
        cluster_rect.h as f32 / char_count.max(1) as f32
    } else {
        cluster_rect.w as f32 / char_count.max(1) as f32
    };
    let is_oversized_single_char = (char_count == 1 && (cluster_rect.w >= oversized_char_limit || cluster_rect.h >= oversized_char_limit))
        || (char_count <= 2 && avg_char_dim >= oversized_char_limit as f32);
